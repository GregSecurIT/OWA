...
if(a.dialogArguments)
 {
var g=new String(Math.round(Math.random()*1e5)),f="toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable=1,width=800,height=600",i=Math.round((screen.availHeight-600)/2),h=Math.round((screen.availWidth-800)/2);
f+=",top="+i+",left="+h;
...
if(d&&(d.g_fOwa||d.g_fEcp))
 {
var j=Math.round((screen.availWidth-800)/2),k=Math.round((screen.availHeight-600)/2);
a.moveTo(j,k);
a.resizeTo(800,600);
...
switch(a)
 {
case gbid("lnkShwSec"):hd(gbid("lnkShwSec"));
shw(gbid("lnkHdSec"));
shw(gbid("trPubExp"));
shw(gbid("trPrvtExp"));
gbid("lnkHdSec").focus();
break;
case gbid("lnkHdSec"):shw(gbid("lnkShwSec"));
hd(gbid("lnkHdSec"));
hd(gbid("trPubExp"));
hd(gbid("trPrvtExp"));
...
document.cookie=secureCookie("logondata=;
...
var b="acc="+(gbid("chkBsc")&&gbid("chkBsc").checked?1:0),c="lgn="+gbid("username").value;
document.cookie=secureCookie("logondata="+b+"&"+c+";
 expires="+a.toUTCString())
}
document.cookie=secureCookie("PBack=0;
 path=/")
}
function clkRtry()
 {
...
function IsOwaPremiumBrowser()
 {
var a=navigator.userAgent,c=navigator.appVersion,h=c.indexOf("Mac")!=-1,i=c.indexOf("Win")!=-1||c.indexOf("NT")!=-1,f=a.indexOf("Trident")!=-1&&a.indexOf("rv:")!=-1,g=f||a.indexOf("MSIE ")!=-1,d=a.indexOf("Firefox/")!=-1&&a.indexOf("Gecko/")!=-1&&Array.every,e=a.indexOf("Safari")!=-1&&a.indexOf("WebKit")!=-1,b=2;
if(g)if(f)b=parseFloat(a.replace(/^.*rv:/,""));
else b=parseFloat(a.replace(/^.*MSIE /,""));
else if(d)b=parseFloat(a.replace(/^.*Firefox\//,""));
else if(e)b=parseFloat(a.replace(/^.*Version\//,""));
else b=parseInt(c);
if(i)
 {
if(g)return b>=7;
else if(e)return b>=3;
else if(d)return b>=3
}
else if(h)if(e)return b>=2;
else if(d)return b>=3;
return false
}
function hres(a)
